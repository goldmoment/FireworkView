...
//  Wildarc
//
//  Created by Vien Van Nguyen on 7/2/16.
...
public class FireworkView: UIView {
    ...
    private func settupLayer(center: CGPoint) {
        ...
        for _ in 0..<numberOfParticle {
            self.layer.addSublayer(createCirle(center,
                radius: radius,
                size: size,
                color: colors[random(min: 0, max: colors.count - 1)],
                duration: duration))
        }
    ...
    private func createCirle(center: CGPoint, radius: Float, size: Float, color: UIColor, duration: Double) -> CAShapeLayer {
        let destinationX = Float(center.x) + random(min: -size / 2, max: size / 2)
        let destinationY = Float(center.y) + random(min: -size / 2, max: size / 2)
        
        let layer = CAShapeLayer()
        let radius = CGFloat(random(min: radius * 0.8, max: radius * 1.2))
    ...
    private func random(min min: Int, max: Int) -> Int {
        return Int(arc4random_uniform(UInt32(max - min + 1))) + min
    }
    
    private func random(min min: Float, max: Float) -> Float {
